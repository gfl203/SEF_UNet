...
# class ScConv(nn.Module):
#     def __init__(self, in_channels, out_channels, group_num=16, gate_threshold=0.5, alpha=1 / 2, squeeze_ratio=2,
#                  group_size=2, group_kernel_size=3):
#         super().__init__()
#
#         # 计算输入通道数，这里假设输入尺寸为 [batch_size, in_channels, height, width]
#         self.in_channels = in_channels
#
#         # 创建SRU和CRU层
#         self.SRU = SRU(in_channels, group_num=group_num, gate_treshold=gate_threshold)
#         self.CRU = CRU(in_channels, alpha=alpha, squeeze_radio=squeeze_ratio, group_size=group_size,
#                        group_kernel_size=group_kernel_size)
#
#         # 创建卷积层，输入通道数为 in_channels
#         self.conv = nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1)
#
#     def forward(self, x):
#         x = self.SRU(x)
#         x = self.CRU(x)
#         x = self.conv(x)  # 将输出传递给卷积层
#         return x

# if __name__ == '__main__':
#     x = torch.randn(1, 16, 16, 16)  # 创建随机输入张量
#     model = ScConv(16)  # 创建 ScConv 模型
#     print(model(x).shape)  # 打印模型输出的形状

    #输入的是通道
# x = torch.randn(1, 16, 16, 16)  # 创建随机输入张量
# model = ScConv(16)  # 创建 ScConv 模型
# print(model)  # 打印模型输出的形状